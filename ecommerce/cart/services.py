...
async def add_to_cart(
    product_id: int, current_user: User, db_session: Session = Depends(db.get_db)
):
    """
    Add to Cart
    """
    ...
    cart_info = (
        ...
        .filter(models.Cart.user_id == user_info.id)
        .first()
    )
...
async def add_items(
    cart_id: int, product_id: int, db_session: Session = Depends(db.get_db)
):
    """
    Add Items
    """
...
async def remove_cart_item(
    cart_item_id: int, current_user: User, db_session: Session
) -> None:
    """
    ...
    user_info = db_session.query(User).filter(User.email == current_user.email).first()
    cart_id = (
        db_session.query(models.Cart).join(User).filter(User.id == user_info.id).first()
    )
    product_object = (
        db_session.query(models.CartItems)
        .filter(models.CartItems.id == cart_item_id)
        .first()
    )
    db_session.query(models.CartItems).filter(
        and_(
            models.CartItems.id == cart_item_id, models.CartItems.cart_id == cart_id.id
        )
    ).delete()
    product_object = db_session.query(Product).filter(
        Product.id == product_object.product_id
    )
    current_quantity = product_object.first().quantity + 1
